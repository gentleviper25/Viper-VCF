...
import { isExpired, timeLeft, formatDate, sessionUrl, copyToClipboard } from '@/lib/utils'
...
export default function SessionCard({ session, onDelete }) {
  ...
  async function handleDownload() {
    setDl(true)
    try {
      const contacts = await fetchContacts(session.session_code)
      // fetchContacts sets state; we need raw data — call supabase directly via hook
      // The hook exposes contacts state, so navigate to results page instead
      navigate(`/dashboard/session/${session.session_code}?download=1`)
    } catch {
      toast({ message: 'Failed to fetch contacts.', type: 'error' })
    }
    setDl(false)
  }
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {session.description && (
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 3, lineHeight: 1.4 }}>
              {session.description.length > 70
                ? session.description.slice(0, 70) + '…'
                : session.description}
            </p>
          )}
        </div>
      ...
      <div style={{
        ...
        <div className="card-sm" style={{ padding: '12px 14px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 2 }}>
            <Users size={13} style={{ color: 'var(--viper)' }} />
            <span style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.4 }}>
              Contacts
            </span>
          </div>
          <p style={{ fontFamily: 'Outfit,sans-serif', fontWeight: 700, fontSize: 22, color: '#fff' }}>
            {count}
          </p>
        </div>
        <div className="card-sm" style={{ padding: '12px 14px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 2 }}>
            <Clock size={13} style={{ color: expired ? '#f87171' : 'var(--viper)' }} />
            <span style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.4 }}>
